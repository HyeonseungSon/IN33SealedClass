/*
Sealed Class
 - Sealed Class 들을 하나로 모아 관리하는 Class 입니다.
 - 열거형은 특정 값을 의미하는 상수들을 모아 관리하는 개념이라면
  Sealed Class 는 객체를 모아 관리하는 개념
 - 열거형의 경우 상수에 지정된 값은 절대로 변경이 불가능 하지만
  Sealed Class 는 객체를 관리하는 개념이므로
  객체가 가지고 있는 변수에 값을 설정하여 지정된 값을 변경하는 것이 가능합니다.
 - 열거형 사용 시 상수가 의미하는 값을 수시로 변경하거나 다양한 형태로 사용하고자 할 때 사용합니다.
 - 열거형의 한계를 극복하는 Class
 */
...
fun main() {
    ...
    val v5 = Number2.SealedThree(300, 400.400)
...
sealed class Number2 {
    ...
    class SealedTwo (val a1:Int) : Number2() {
        fun sealedFun2 () {
            println("SealedTwo 의 sealedFun2 입니다.")
        }
    }
